<template>
    <div class="inboundUpdate_background">
        ...
        <div class="inboundUpdate_container">
            <div class="inboundUpdate_header">
                ...
                <el-button type="primary" plain @click="batchModification">批量设置入库状态</el-button>
            </div>
            <div></div>
            <div class="inboundUpdate_SubContainer">
                ...
                >
                    ...
                    <el-table-column label="零件号" prop="itemNum" width="128"/>
                    ...
                    <el-table-column label="入库状态" width="144">
                        <template #default="scope">
                            <el-select
                            ...
                            >
                                <el-option
                                    v-for="status in inboundStatus.statusArr"
                                    :key="status.value"
                                    :label="status.key"
                                    :value="status.value"
                                >
  ...
  import inboundStatus from '../router/inboundStatus.js';
  ...
  export default {
    name: "Inbound",
      ...
      setup(props) {
          ...
          const inboundStatusReactive = reactive(inboundStatus);
          ...
          const putInboundDetail = () => {
            console.log('要发送请求了')
              axios.put('http://localhost:8080/inboundDetail', {
                  inboundNum: inboundNum.value,
                  inboundDetail: tableData.map(el => { return {
                      itemNum: el.itemNum,
          ...
          const changeStatus = (obj,val) => {
              obj.itemStatus = val;
              obj.itemStatusName = inboundStatus.statusArr[val].key
              console.log(obj,val)
          }
          ...
          const handleNavClose = function() {
              let itemToClose = { title:'入库', path:`/inbound/${inboundNum.value}` }
                      ...
                      [
                          ...
                          h(
                              BoxElSelect,
                              {
                                  ...
                                  statusArr: inboundStatus.statusArr
                              }
                          )
                      ]
                  )
          ...
          onBeforeMount(()=>{
            ...
            router.afterEach((to, from) => {
                let num = router.currentRoute.value.params.inboundNum;
                if (num) {
                  inboundNum.value = num;
                }
            });
          })

          onMounted(() => {
                const num = router.currentRoute.value.params.inboundNum;
                if (num) {
                  inboundNum.value = num;
                }
              console.log(currentStatus.value);
              axios.get(`http://localhost:8080/inboundDetail/${inboundNum.value}`)
                  .then(res => {
                      let result = res.data;
                      tableData.splice(0);
                      minQuantity.value = [];
                      result.forEach((el, index) => {
                          ...
                          tableData[index].itemStatusName = inboundStatus.statusArr[tableData[index].itemStatus].key;
          ...
          return {
              ...
              inboundNum,
              choosenStatus,
              inboundStatus: inboundStatusReactive,
              currentStatus,
              choosenRows,
              minQuantity,
              tableData,
