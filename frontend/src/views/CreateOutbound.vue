<template>
    <el-form :model="form" label-width="auto" style="max-width: 100%" :rules="formrules" ref="formref">
        ...
        <el-form-item label="客户" prop="sup">
            <el-select v-model="form.sup" placeholder="请选择客户" @change="clearCpTable">
      ...
      font-size: 24px;">零件明细</span>
        ...
        <el-form-item >
            <el-table :data="cpTable" style="width: 100%" :cell-style="{ textAlign: 'center' }"
                ...
                <el-table-column prop="innum" label="出库数量" width="180">
    ...
    <el-dialog v-model="diavis" width="650">
        <el-form :model="form">
            <el-form-item label="供应商">
                <el-select v-model="newsup" placeholder="请选择供应商" >
                <el-option v-for="item in newsups" :key="item.name" :label="item.name" :value="item.name" />
                </el-select>
            </el-form-item>
...
let newsups:Ref<{name:string}[]> = ref([{
    name:"All"
}])
let newsup =ref("All")
let Cps: Ref<Cpb[]> = ref([])

axios.get(url+'customer').then(function({data}){
    for(let cus of data){
        sups.value.push({name:cus.customerCode})
    }
})
axios.get(url+'item').then(function({data}){
    ...
    let flag=true
    for(let sup of newsups.value){
        if(sup.name==cp.sup){
            flag=false
        }
    }
    if(flag)
    newsups.value.push({name:cp.sup})
}
})
let slctno = ref('')
let slctnum = ref(1)
        ...
        if (valid) {
            ...
            if(form.cpData.length>0){
               let pCps:({
                ...
                let  data={
                    "outboundNum":form.ibno,
                    "customer":form.sup,
                    "outboundDetail":pCps
                }
                axios.post(url+'outbound',data)
                 props.closeNav({title:"创建出库单",path:"/createOutbound"})
              console.log('submit!')
            }
            else{
...
const screenCp = () => {
    ...
    if(newsup.value!="All"){
        cpBTable.value=cpBTable.value.filter(function(v){return v.sup==newsup.value})
    }
}
const addCp = () => {
    cpBL.value = Cps.value
